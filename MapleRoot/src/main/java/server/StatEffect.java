...
public class StatEffect {
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case 5221018:
                    ...
                    statups.add(new Pair<>(BuffStat.STANCE, iprop));
                    statups.add(new Pair<>(BuffStat.COMBO_BARRIER, 500));
                    break;
    ...
    public void rolltheDice(Character applyto)
    {
        Random random = new Random();
        int roll = random.nextInt(12) + 1;;
        switch (roll) {
            case 1, 2 -> statups.add(new Pair<>(BuffStat.WATK, 0));
            case 3, 6, 9 -> statups.add(new Pair<>(BuffStat.WATK, 15 * roll));
            case 5 -> statups.add(new Pair<>(BuffStat.SHARP_EYES, 40 << 8 | 20));
            case 4 -> statups.add(new Pair<>(BuffStat.COMBO_BARRIER, 800));
            case 7 -> statups.add(new Pair<>(BuffStat.ECHO_OF_HERO, 50));
            case 8 ->  statups.add(new Pair<>(BuffStat.SPEED, 300));
            case 10 -> statups.add(new Pair<>(BuffStat.SPEED_INFUSION, -6));
            case 11, 12 -> statups.add(new Pair<>(BuffStat.SHADOWPARTNER, 100));
        }
        switch (roll) {
            case 1, 2 -> applyto.dropMessage("Rolled: " + roll + " and nothing happened.");
            case 3, 6, 9 -> applyto.dropMessage("Rolled: " + roll + ". Attack increased by: " + roll * 15);
            case 4 -> applyto.dropMessage("Rolled: " + roll + ". Damage is reduced by 80%");
            case 5 -> applyto.dropMessage("Rolled: " + roll + ". Critical Rate increased by 40%");
            case 7 -> applyto.dropMessage("Rolled lucky 7! Attack increased by 50%");
            case 8 -> applyto.dropMessage("Rolled: " + roll + ". Gain 300% speed.");
            case 10 -> applyto.dropMessage("Rolled: " + roll + ". Cap attack speed.");
            case 11, 12 -> applyto.dropMessage("Rolled: " + roll + ". High roller! Deal bullet damage twice.");
        }
    }
    ...
    private boolean applyTo(Character applyfrom, Character applyto, boolean primary, Point pos, boolean useMaxRange, int affectedPlayers) {
        ...
        if (summonMovementType != null && pos != null) {
            if (applyto.arrowplatterrunning && sourceid == 3121013 && applyto.getReborns() == 0) {
                return false;
            }
            if (applyto.arrowplatterrunning && applyto.arrowplatterrunning1 && sourceid == 3121013 && applyto.getReborns() == 1) {
                return false;
            }
            if (applyto.arrowplatterrunning && applyto.arrowplatterrunning1 && applyto.arrowplatterrunning2 && sourceid == 3121013 && applyto.getReborns() == 2) {
                return false;
            }
            if (applyto.arrowplatterrunning && applyto.arrowplatterrunning1 && applyto.arrowplatterrunning2 && applyto.arrowplatterrunning3 && sourceid == 3121013 && applyto.getReborns() == 3) {
                return false;
            }
            final Summon tosummon = new Summon(applyfrom, sourceid, pos, summonMovementType);
            applyfrom.getMap().spawnSummon(tosummon);
            if (sourceid == 3121013) {
                applyfrom.addSummon(applyfrom.currsummonid, tosummon);
            } else {
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - Server.getInstance().getCurrentTime()));
    ...
    private void applyBuffEffect(Character applyfrom, Character applyto, boolean primary) {
        ...
        if (isMonsterRiding()) {
            ...
            if (sourceid == Corsair.BATTLE_SHIP) {
                ...
                if(!applyto.battleshipRunning) {
                    System.out.println("catch me outside how bout that");
                    applyto.runBattleShipSchedule();
                }
    ...
    public boolean isArrowPlatter() {
        return skill && (sourceid == 3121013);
    }
