    ...
    public synchronized void visitShop(Character chr) {
        ...
        try {
            if (this.isOwner(chr)) {
                ...
                if (!existingItems.isEmpty() && ItemFactory.MERCHANT.loadItems(chr.getId(), false).isEmpty()) {
                    String itemString = "";
                    if (this.getItems() != null && !this.getItems().isEmpty()) {
                        for (PlayerShopItem mpsi : this.getItems()) {
                            itemString += mpsi.getItem().getItemId() + ":" + mpsi.getItem().getQuantity() + "\n";
                        }
                    }
                    client.autoban.AutobanFactory.PACKET_EDIT.alert(chr, "tried to packet edit (dupe) with merchant and Fredrick [m]. This could be a false positive, the data has been logged.");
                    tools.FilePrinter.print(tools.FilePrinter.FREDRICK,
                            "[" + new SimpleDateFormat("HH:mm:ss").format(new Date()) + "]" + chr.getName() + " tried to access merchant with empty items after retrieving from Fredrick. Items: " + itemString);
                    this.clearItems();
                    chr.getClient().disconnect(false, false);
                    //chr.autoban("Packet Editing (mf)");
                }
