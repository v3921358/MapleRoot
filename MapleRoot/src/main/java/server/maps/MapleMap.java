...
public class MapleMap {
    ...
    private int magnusSpawnCounter = 0;
    ...
    public boolean damageMonster(final Character chr, final Monster monster, final long damage) {
        ...
        if (monster.getId() == 8880002) {
            double hpRatio = (double) monster.getHp() / monster.getMaxHp();
            if (hpRatio <= 0.50 && magnusSpawnCounter == 0) {
                atomLimit += 20;
                magnusSpawnCounter++;
            } else if (hpRatio <= 0.25 && magnusSpawnCounter == 1) {
                atomLimit += 30;
                magnusSpawnCounter++;
            } else if (hpRatio <= 0.05 && magnusSpawnCounter == 2) {
                atomLimit += 100;
                magnusSpawnCounter++;
            }
        }
...
/*    public void broadcastBalrogVictory(String leaderName) {
    ...
    */
    ...
    void spawnAtoms(int difficulty) {
        if (difficulty == 0) {
            atomLimit = 30;
        } else if (difficulty == 1 && magnusSpawnCounter < 1) {
            atomLimit = 40;
        }
        int TO_SPAWN = atomLimit - countMonsters();
        int GREEN_ATOM_MIN = 8889004;
        int GREEN_ATOM_MAX = 8889006;
        int BLUE_ATOM_MIN = 8889007;
        int BLUE_ATOM_MAX = 8889009;
        int PURPLE_ATOM_MIN = 8889010;
    ...
    public void moveMonster(Monster monster, Point reportedPos) {
        ...
        if (mapid == 401060100) {
            spawnAtoms(0);
        } else if (mapid == 401060200 || mapid == 401060101) {
            spawnAtoms(1);
        } else if (mapid == 401060300) {
            spawnAtoms(2);
        }
